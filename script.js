...
// Timezone offset in hours for "Horário Padrão do Amazonas" is -4. Define as global constant
...
function rehydrateTargets(targets) {
    return targets.map(target => {
        if (target.date) {
            console.log("Rehydrating date - Before:", target.date); // Log before rehydration
            target.date = (target.date instanceof Timestamp) ? target.date.toDate() : new Date(target.date);
            console.log("Rehydrated date - After:", target.date); // Log after rehydration
        }
        if (target.archivedDate) target.archivedDate = (target.archivedDate instanceof Timestamp) ? target.archivedDate.toDate() : new Date(target.archivedDate);
        if (target.deadlineDate) target.deadlineDate = (target.deadlineDate instanceof Timestamp) ? target.deadlineDate.toDate() : new Date(target.deadlineDate);
        if (target.observations) target.observations.forEach(obs => obs.date = (obs.date instanceof Timestamp) ? obs.date.toDate() : new Date(obs.date));
        return target;
    });
}
...
function renderTargets() {
  ...
  targetsToDisplay.forEach((target) => {
    ...
    console.log("Target Title:", target.title, "Original Date (Timestamp):", target.date, "Formatted Date for Display:", formattedDate); // Log before display
...
function renderResolvedTargets() {
    ...
    const endIndex = currentResolvedPage + targetsPerPage;
    const targetsToDisplay = filteredTargets.slice(startIndex, endIndex);
...
async function saveObservation(targetId) {
    ...
    if (observationText !== "") {
        ...
        let timestampObservationDate;
        if (observationDateValue) {
            const [obsYear, obsMonth, obsDay] = observationDateValue.split('-').map(Number);

            // Timezone offset correction for observation date
            const obsUtcDate = new Date(Date.UTC(obsYear, obsMonth - 1, obsDay));
            const localObservationDate = new Date(obsUtcDate.getTime() + timezoneOffsetHours * 60 * 60 * 1000);
            timestampObservationDate = Timestamp.fromDate(localObservationDate);
             // ==== LOGGING - Observation Date
            console.log("Observation Input Value (YYYY-MM-DD):", observationDateValue);
            console.log("JavaScript Date Object (LocalObservationDate) with Timezone Offset Correction BEFORE Timestamp Conversion:", localObservationDate);
            console.log("Firebase Observation Timestamp (timestampObservationDate) AFTER fromDate():", timestampObservationDate);
            // ==== LOGGING - End Observation Date


        } else {
            // Timezone offset correction for "now" date if no date is picked
            const nowUtcDate = new Date(Date.UTC(new Date().getUTCFullYear(), new Date().getUTCMonth(), new Date().getUTCDate()));
            const localObservationDate = new Date(nowUtcDate.getTime() + timezoneOffsetHours * 60 * 60 * 1000);
            timestampObservationDate = Timestamp.fromDate(localObservationDate);
             // ==== LOGGING - Observation Date (default "now")
            console.log("No Observation Date Input, using 'now'");
            console.log("JavaScript Date Object (LocalObservationDate) with Timezone Offset Correction BEFORE Timestamp Conversion:", localObservationDate);
            console.log("Firebase Observation Timestamp (timestampObservationDate) AFTER fromDate():", timestampObservationDate);
            // ==== LOGGING - End Observation Date
        }
        let observationDate = timestampObservationDate; // Use the Timestamp
        ...
        try {
            ...
            if (targetDoc.exists()) {
                ...
                updatedObservations.push({ date: observationDate, observation: observationText }); // Save Timestamp
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    // ==== LOGGING - START of Date Input Handling ====
    const dateInputValue = document.getElementById("date").value;
    console.log("Data Input Value (YYYY-MM-DD):", dateInputValue);

    const [year, month, day] = dateInputValue.split('-').map(Number);
    console.log("Year:", year, "Month:", month, "Day:", day);

    // Timezone offset correction for Target Date
    const timezoneOffsetHours = 4;
    const utcYear = year;
    const utcMonth = month - 1; // Month is 0-indexed in JavaScript and UTC
    const utcDay = day;

    const utcDate = new Date(Date.UTC(utcYear, utcMonth, utcDay)); // Create Date object in UTC
    const localDate = new Date(utcDate.getTime() + timezoneOffsetHours * 60 * 60 * 1000); // Create a *local* Date object from the UTC Date
    console.log("JavaScript Date Object (LocalDate) with Timezone Offset Correction BEFORE Timestamp Conversion:", localDate);
    console.log("LocalDate toISOString():", localDate.toISOString()); // ISO String representation

    const timestampDate = Timestamp.fromDate(localDate);
    console.log("Firebase Timestamp (timestampDate) AFTER fromDate():", timestampDate);
    console.log("Timestamp toDate() - Back to Date Object:", timestampDate.toDate());
    console.log("Timestamp toDate().toISOString():", timestampDate.toDate().toISOString());
    // ==== LOGGING - END of Date Input Handling ====


    const deadlineDateInputValue = document.getElementById("deadlineDate").value;
    let timestampDeadline = null;
    if (hasDeadline && deadlineDateInputValue) {
        const [deadlineYear, deadlineMonth, deadlineDay] = deadlineDateInputValue.split('-').map(Number);

        // Timezone offset correction for Deadline Date as well
        const deadlineUtcDate = new Date(Date.UTC(deadlineYear, deadlineMonth - 1, deadlineDay));
        const localDeadline = new Date(deadlineUtcDate.getTime() + timezoneOffsetHours * 60 * 60 * 1000);
        timestampDeadline = Timestamp.fromDate(localDeadline);

         // ==== LOGGING - Deadline Date
        console.log("Deadline Input Value (YYYY-MM-DD):", deadlineDateInputValue);
        console.log("JavaScript Date Object (LocalDeadline) with Timezone Offset Correction BEFORE Timestamp Conversion:", localDeadline);
        console.log("Firebase Deadline Timestamp (timestampDeadline) AFTER fromDate():", timestampDeadline);
        // ==== LOGGING - End Deadline Date
    }
    ...
    const newTarget = {
        ...
        date: timestampDate, // Use timestampDate
        resolved: false,
        observations: [],
        hasDeadline: hasDeadline,
        deadlineDate: timestampDeadline, // Use timestampDeadline
        userId: userId
    };
    ...
    try {
        ...
        console.log("Document written with date:", newTarget.date); // Log Timestamp when writing
        ...
        if (user) {
            ...
            const targetDoc = await getDoc(targetRef); // Use getDoc here
             ...
             if (targetDoc.exists()) {
                ...
                const resolvedTargetData = {...targetDoc.data(), resolved: true, archivedDate: Timestamp.fromDate(new Date())}; // Save archivedDate as Timestamp
...
async function archiveTarget(targetId) {
    try {
        ...
        if (user) {
            ...
            if (targetDoc.exists()) {
                //Não precisa do userId aqui, já estamos na coleção do usuário correto
                 const archivedTargetData = {...targetDoc.data(), archivedDate: Timestamp.fromDate(new Date())}; // Save archivedDate as Timestamp
                const archivedRef = doc(collection(db, "users", user.uid, "archivedTargets"));// Usa o UID do usuário
            ...
            if (target.observations && target.observations.length > 0) {
                ...
                target.observations.forEach(obs => {
                    ...
                    if (obs.date instanceof Timestamp) {
                        obsDate = obs.date.toDate(); // Converter Timestamp para Date
                    } else {
                        obsDate = new Date(obs.date); // Se já for Date, usar diretamente ou tentar converter
                    }
                    htmlContent += `<p><strong>${formatDateForDisplay(obsDate)}:</strong> ${obs.observation}</p>`;
                });
            }
            htmlContent += '</div><hr>';
        });
...
function generateResolvedViewHTML(startDate, endDate) {
    ...
    const filteredResolvedTargets = resolvedTargets.filter(target => {
        ...
        const resolvedDateObj = (target.archivedDate instanceof Timestamp) ? target.archivedDate.toDate() : new Date(target.archivedDate);
...
async function editDeadline(targetId) {
    ...
    try {
        ...
        if (user) {
            const targetRef = doc(db, "users", user.uid, "prayerTargets", targetId);
            await updateDoc(targetRef, { deadlineDate: Timestamp.fromDate(new Date(convertToISO(newDeadline) + "T00:00:00")) }); // Save deadline as Timestamp
...
function convertToISO(dateString) {
    const parts = dateString.split('/');
    return `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`; // Correctly formatted ISO
}
...
async function loadPerseveranceData(userId) {
    ...
    if (docSnap.exists()) {
        ...
        if (perseveranceData.lastInteractionDate) {
            let lastInteractionDate = perseveranceData.lastInteractionDate.toDate(); // Convert Firestore Timestamp to Date for comparison
            lastInteractionDate.setHours(0, 0, 0, 0);

            if (lastInteractionDate.getTime() < yesterday.getTime()) {
        ...
        await setDoc(perseveranceRef, perseveranceData);
    }
