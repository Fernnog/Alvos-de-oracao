...
function rehydrateTargets(targets) {
    return targets.map(target => {
        if (target.date) target.date = (target.date instanceof Timestamp) ? target.date.toDate() : new Date(target.date);
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    const newTarget = {
        ...
        date: Timestamp.fromDate(new Date(document.getElementById("date").value + "T00:00:00")), // Use Timestamp for date
        resolved: false,
        observations: [],
        hasDeadline: hasDeadline,
        deadlineDate: hasDeadline ? Timestamp.fromDate(new Date(document.getElementById("deadlineDate").value + "T00:00:00")) : null, //Use Timestamp for deadline
        userId: userId
    };
