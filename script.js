...
async function saveObservation(targetId) {
    ...
    if (observationText !== "") {
        let observationDate = observationDateValue ? Timestamp.fromDate(new Date(observationDateValue + "T00:00:00")) : Timestamp.fromDate(new Date()); // Use Timestamp for observationDate
...
document.getElementById("prayerForm").addEventListener("submit", async (e) => {
    ...
    // Using Date.UTC to create UTC date first, then local Date
    const utcYear = year;
    ...
    const localDate = new Date(utcDate); // Create a *local* Date object from the UTC Date
    console.log("JavaScript Date Object (LocalDate) using UTC method BEFORE Timestamp Conversion:", localDate);
    ...
    if (hasDeadline && deadlineDateInputValue) {
        ...
        const localDeadline = new Date(deadlineYear, deadlineMonth - 1, deadlineDay);
        ...
        console.log("JavaScript Date Object (LocalDeadline) BEFORE Timestamp Conversion:", localDeadline);
